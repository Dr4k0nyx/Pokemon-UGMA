...
export default async function downClassroom_2(k) {
  colorizeBackground(k, 58, 58, 80);

  const mapData = await fetchMapData("./assets/maps/salonAbajo2.json");
  ...
  entities.player.onCollide("classroom - exit", () => {
    gameState.setPreviousScene("DownClassroom_2");
    k.go("firstFloor");
  });
