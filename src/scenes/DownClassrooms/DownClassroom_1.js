...
export default async function downClassroom_1(k) {
  colorizeBackground(k, 58, 58, 80);

  const mapData = await fetchMapData("./assets/maps/salonAbajo1.json");
  ...
  entities.player.onCollide("classroom - exit", () => {
    gameState.setPreviousScene("DownClassroom_1");
    k.go("world");
  });
