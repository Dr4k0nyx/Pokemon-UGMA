...
export default async function rightDownClassroom(k) {
  colorizeBackground(k, 58, 58, 80);

  const mapData = await fetchMapData("./assets/maps/salonAbajoDer.json");
  ...
  entities.player.onCollide("rightDownClassroom - exit", () => {
    gameState.setPreviousScene("rightDownClassroom");
    k.go("world");
  });
